...
def hough_transform(x: int, y: int, w: int, h: int, image: np.ndarray, output=None):
    if output is None:
        output = image.copy()
    ...
    if lines is not None:
        ...
        for rho, theta in lines[0]:
            ...
            angle_deg = (theta * 180 / np.pi + 90) % 180

            if 0 <= angle_deg <= 180:
                 cv2.putText(output, f"{angle_deg:.2f}", (10, 50),
...
def main():
    ...
    classifier = Path(__file__).with_name("pen_vertical_classifier.xml")
    if not classifier.is_file():
        raise FileNotFoundError(f"Missing pen classifier: {classifier}")
    pencasc_vert = cv2.CascadeClassifier(str(classifier))
    if pencasc_vert.empty():
        raise ValueError(f"Invalid classifier: {classifier}")
    try:
        with Button(6) as red_button, Picamera2() as picam2:
            ...
            while True:
                ...
                for x, y, w, h in pens_vert:
                    hough_transform(x, y, w, h, bgr_frame, detection_frame)
                bgr_frame = detection_frame
                for x, y, w, h in pens_vert:
