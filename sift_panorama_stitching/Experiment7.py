...
def capture_sequence(
    ...
    with ExitStack() as stack:
        picam2 = stack.enter_context(Picamera2())
        ...
        try:
            ...
            green_button = stack.enter_context(Button(5))
        ...
        try:
            ...
            while frames_remaining > 0:
                ...
                if green_button.is_pressed:
                    if not is_pressed:
                        frames.append(bgr_frame[..., :3].copy())
                        frames_remaining -= 1
...
def create_panorama(
    ...
    if img1.size == 0 or img2.size == 0:
        raise ValueError("Empty panorama input")
    sift = cv2.SIFT_create(features)
    kp1, des1 = sift.detectAndCompute(cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY) if img1.ndim == 3 else img1, mask)
    kp2, des2 = sift.detectAndCompute(cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY) if img2.ndim == 3 else img2, mask)
    ...
    if len(good_matches) >= 4:
        ...
        raise ValueError("Not enough matches to stitch")
...
def main():
    ...
    try:
        if DEBUG:
            thread = threading.Thread(target=show_free_memory, args=(stop_event,), daemon=True)
            thread.start()
        imgs = interface()
        if not imgs or len(imgs) < 2:
            print("At least two captured images are required.")
            return
        _, _, _, result = create_panorama(imgs[0], imgs[1])
        for next_img in imgs[2:]:
            _, _, _, result = create_panorama(result, next_img)
        if not cv2.imwrite("panorama.png", result):
            raise OSError("Could not save panorama.png")
    except (ValueError, cv2.error) as exc:
        print(f"Panorama failed: {exc}")
    finally:
        stop_event.set()
